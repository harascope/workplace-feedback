...
import { analyze, deliver, openAdmin, openUser, pendingStat, resetDemo, sendFeedback } from "./helpers";
...
test("送信後も完了画面が残り、取り消すと未配信件数が元に戻る", async ({ page, context }) => {
  ...
  await expect(page.getByRole("button", { name: "取り消す", exact: true })).toHaveCount(0);
...
test("本文に書かれた名前から宛先が自動で選ばれる", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "送る", exact: true })).toBeEnabled();
  ...
  await expect(page.getByRole("button", { name: "送る", exact: true })).toBeDisabled();
});
