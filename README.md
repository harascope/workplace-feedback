...
- **デモ用データ。** 管理者画面の「デモ用データを入れる」（`POST /admin/seed-demo`）で、
  名簿6人ぶんの申告22件・引き継ぎ2件を入れ直す。複数部署・複数重大度・配信済み／未配信が混ざる。
  フッターの「デモを初期状態に戻す」（`POST /admin/reset`）とは別経路で、reset の件数は変えない
...
- 対象者を特定しない申告（組織単位の記録、2.6）。`src/components/Compose.tsx` は宛先が必須で、
  対象者なしで送る経路が無い。このため**部署アラート（6.2）は発火の入力を代用している**：
  本来の「対象者が特定されない申告の集積」ではなく、その部署あての配信済み申告を見て、
  異なる申告者が2人以上・複数名に分散しているときに出す（件数と原文は出さない）
- 部署評価の経時変化（前月比）と人数あたり申告率（6.1 の補助軸）。`evaluate_depts` は
  重大度分布・件数の閾値・集中度の3つだけで出している
- 最後に配信した日時。`reports` に配信時刻の列が無いため、管理者画面には「次の配信」と
  「いちばん古い未配信が何日待っているか」だけを出している
