...
相手は認知できない）
   ...
   話し方・接し方の様態も行動として扱う。相手がどう話したか・どう接したかを名指した記述は、\
「その様態で話す／接する」という行動に直して description に書く。\
その場に居合わせた人が見聞きできるので observable は true。
   - 「口調がきびしすぎる」→「きびしい口調で話す」
   - 「言い方がきつい」→「きつい言い方をする」
   - 「威圧的な話し方をされる」→「威圧的な話し方をする」
   - 「態度が冷たい」→「冷たい態度で接する」
   ただし良し悪しを言うだけの語は、様態を名指していないので行動にしない。\
「きびしい・きつい・冷たい・威圧的・そっけない」は様態、「悪い・ひどい・最低」は評価。

   評価と行動が混ざっている場合（見下すような言い方をされた）は、行動部分だけを description に残す。
   相手の行動も様態も書かれておらず、書き手の感情や好悪だけの場合（なんかつらい、あの人が嫌い）は、\
actions を空配列にする。

   身体的特徴・容姿・体臭など、相手が変えられない/行動でない属性への言及だけの場合も、\
actions は空配列にする（人格攻撃を行動として誤って通してはならない）。ただしこの場合に限り、\
rephrase_hint に「本人に伝えても改善されない」という趣旨の行動への言い換え案を1文で出す。\
例:「部長の足が臭すぎる」→「（本人に）指摘しても改善されない」。\
この言い換え案は actions には入れない。送信者が書き直すかどうかを選べるようにするための\
提案であり、システムが代わりに断定してはならない。\
感情や好悪だけの入力（なんかつらい等）には rephrase_hint を出さない（言い換える対象が無いため）。
...
{{
  ...
  "organized": "整えた文面",
  "rephrase_hint": "行動への言い換え案" または null
}}
