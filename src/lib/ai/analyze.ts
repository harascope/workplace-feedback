...
import { isStubMode, stubAnalyze } from "./stub";

export type AnalyzeInput = {
  /** 送信者の自由記述。原文のまま渡す */
  body: string;
  /** 名簿の候補氏名。targetHint の照合に使う */
  candidateNames: string[];
};

const prompt = ({ body, candidateNames }: AnalyzeInput) =>
  `あなたは職場フィードバックツールの解析エンジンです。従業員が書いた訴えを解析し、JSONのみを返してください。前置き・説明・マークダウンは一切出力しないこと。

## 解析の観点

1. actions: 相手が「した」こと。判定軸は外から観察できるかどうか。
   - 観察できる: 遮る、言う、送る、無視する、割り当てる、返さない
   - 観察できない: 態度が悪い、感じが悪い、やる気がない、私を嫌っている（これは書き手の評価であり、相手は認知できない）
...
3. targetHint: 文中に相手の名前があれば返す。候補: ${candidateNames.join("、")}。無ければ null。

4. severity:
   - 3 = 暴行・傷害、性的強要・わいせつ行為、脅迫、ストーカー行為、自死をほのめかす記述。犯罪または緊急対応の領域。
...
/** 行動・場面・対象者の抽出、重大度判定、特定リスク判定を 1 回の呼び出しでまとめて返す。 */
export async function analyze(input: AnalyzeInput): Promise<Analysis> {
  if (isStubMode()) return stubAnalyze(input.body, input.candidateNames);
  return callStructured(analysisSchema, prompt(input), { maxTokens: 1200 });
}

/** 観察可能な行動が取れているか。分岐（仕様書 2.6）の判定に使う */
export function hasObservableAction(a: Analysis): boolean {
  return a.actions.some((x) => x.observable);
}
